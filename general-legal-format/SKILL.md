...
Because this skill is **explicit-only**, it will only activate when Ryan invokes `/general-legal-format`. It will NOT auto-trigger on phrases like "format this like the template" or "use the legal format."
...
**Ask Ryan for the file name every time this skill is invoked.** Do not assume a naming convention. Suggest a format like `[Project Name] - [Document Name] - MMDDYYYY [version].docx` but let Ryan confirm or override.
